 ...
 * @method getChannelsLowerCasedRegExp
 * @returns {StreamChannels.channels}
 */
StreamChannels.prototype.getChannelsLowerCasedRegExp = function() {
  return this.channelsLowerCasedRegExp;
};
...
var helpers = {
  ...
  preprocessKeywords: function(options, streamChannels) {
    ...
    streamChannels.channelsLowerCasedRegExpSafe = {};
    streamChannels.channelsLowerCasedRegExp = {};
    streamChannels.channelsKeywordMapping = {};//between normal case and lowercase
    ...
    for (var channel in options.track) {
      ...
      streamChannels.channelsKeywordMapping[channel] = {};
      streamChannels.channels[channel].forEach(function(item){
        streamChannels.channelsKeywordMapping[channel][item.toLowerCase()] = item;
      });
      streamChannels.channelsLowerCasedRegExpSafe[channel] = streamChannels.channelsLowerCased[channel].map(function(item){
        return regExpEscape(item);//escape the lower cased keywords so they will be regexp proof
      });
      streamChannels.channelsLowerCasedRegExp[channel] = streamChannels.channelsLowerCased[channel].length > 0 ? new RegExp(streamChannels.channelsLowerCasedRegExpSafe[channel].join('|'),'g') : null;//create the full text search regexp on lower cased keywords
      streamChannels.channels[channel].forEach(function(item){
  ...
  /*
   * Tags $channels and $keywords in the tweet
   * (Returns an object of array of channelsName and keywordsName)
   ...
   * @returns {Object} tweetInfos Containing two arrays : channels and keywords found in the tweet
   */ 
   ...
   * @param {type} tweetInfos infos from postProcessed tweet
   ...
   */
  emitPosprocessedTweet: function(tweet,tweetInfos,streamChannels){
  ...
  onTweetEvent: function(tweet, streamChannels){
    var tweetInfos = helpers.postprocessTweet(tweet, streamChannels);
    return helpers.postprocessTweet(tweet, tweetInfos, streamChannels);
  }
};
